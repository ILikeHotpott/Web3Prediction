...
def _recompute_option_probs(option_states: List[AmmPoolOptionState], b: float, now):
    """
...
def execute_buy(
    *,
    ...
    with transaction.atomic():
        ...
        _, option_states, pool_state = _lock_pool_state(market_id)
        ...
        # Update AMM state q (only target outcome changes)
        ...
        _recompute_option_probs(option_states, pool_state.b, now)
...
def execute_sell(
    *,
    ...
    LOCK ORDER (must match execute_buy):
      ...
      4) Position
    """
    if shares is None and desired_amount_out is None:
        raise ExecutionError("shares or desired_amount_out is required", code="INVALID_PARAM", http_status=400)
    ...
    with transaction.atomic():
        ...
        if Decimal(position.shares) < shares_to_sell:
